...
fn temp_config_path() -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .as_millis();
    std::env::temp_dir()
        .join(format!("voicewin-live-replay-{nonce}"))
        .join("config.json")
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let prompt_text = std::env::var("VOICEWIN_LIVE_PROMPT_TEXT").unwrap_or_else(|_| {
        "Fix grammar, punctuation, and capitalization. Return only the cleaned text.".into()
    });
    let transcript = std::env::var("VOICEWIN_LIVE_TRANSCRIPT")
        .unwrap_or_else(|_| "voice win uses eleven labs scribe v2 for dictation cleanup".into());
    ...
    let app_process_name =
        std::env::var("VOICEWIN_LIVE_APP_PROCESS").unwrap_or_else(|_| "code".into());
    let app_window_title =
        std::env::var("VOICEWIN_LIVE_APP_WINDOW").unwrap_or_else(|_| "Inbox".into());
    ...
    defaults.context.use_custom_vocabulary = custom_vocabulary.is_some();
    ...
    let prompt = PromptTemplate {
        ...
        title: std::env::var("VOICEWIN_LIVE_PROMPT_TITLE")
            .unwrap_or_else(|_| "History Replay".into()),
    ...
    svc.save_config(&cfg).context("save live replay config")?;
    ...
    let app = AppIdentity::new()
        .with_process_name(app_process_name.clone())
        .with_window_title(app_window_title.clone());
    ...
    for round in 0..rounds {
        ...
        let response = svc
            .preview_prompt_with_app_snapshot(
                ...
                app.clone(),
                ContextSnapshot {
                    window_context: Some(format!(
                        "Application: {app_process_name}\nActive Window: {app_window_title}"
                    )),
                    screenshot: screenshot_data_url.as_ref().map(|data_url| ImageArtifact {
            ...
            .await
            .with_context(|| format!("run live replay preview round {}", round + 1))?;
    ...
    println!(
        "app_window_title={}",
        response.app_window_title.as_deref().unwrap_or("")
    );
    println!(
