...
use voicewin_providers::gemini::{
    GeminiGenerateContentConfig, build_list_models_request, build_stream_generate_content_request,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let rounds = std::env::var("VOICEWIN_GEMINI_ROUNDS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(5);
    let warmup_delay_ms = std::env::var("VOICEWIN_GEMINI_WARMUP_DELAY_MS")
        .ok()
